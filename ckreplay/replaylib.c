...
static bool replayline(ck_ht_t *htp, replay_stats_t* statsp, const uchar* buffer, size_t buffersz);

static bool replay_malloc(ck_ht_t *htp, replay_stats_t* statsp, const uchar* buffer, size_t buffersz);

static bool replay_calloc(ck_ht_t *htp, replay_stats_t* statsp, const uchar* buffer, size_t buffersz);

static bool replay_realloc(ck_ht_t *htp, replay_stats_t* statsp, const uchar* buffer, size_t buffersz);

static bool replay_free(ck_ht_t *htp, replay_stats_t* statsp, const uchar* buffer, size_t buffersz);
...
int process_file(const char *filename, bool verbose){
  ...
  if (!table_init(&allocator, &ht, 1024)) {
    fprintf(stderr, "Could not initialize the ck hashtable: %s\n", strerror(errno));
    return 1;
  }
  ...
  while(readline(fp, buffer, BUFFERSZ)) {
    ...
    if (!replayline(&ht, &stats, buffer, BUFFERSZ)) {
...
static bool replayline(ck_ht_t *htp, replay_stats_t* statsp, const uchar* buffer, size_t buffersz) {
  ...
  switch(opchar) {
  case 'm': {
    retval = replay_malloc(htp, statsp, buffer, buffersz); 
    break;
  }
  case 'f': {
    retval = replay_free(htp, statsp, buffer, buffersz); 
    break;
  }
  case 'c': {
    retval = replay_calloc(htp, statsp, buffer, buffersz); 
    break;
  }
  case 'r': {
    retval = replay_realloc(htp, statsp, buffer, buffersz); 
    break;
  }
...
static bool replay_malloc(ck_ht_t *htp, replay_stats_t* statsp, const uchar* buffer, size_t buffersz) {
  ...
  if (success) {
    ...
    /* could assert that key is not in the htp */
    if ( ! table_insert(htp, key, val) ) {
      fprintf(stderr, "Could not insert %p => %p into the htp: %s\n", key, val, strerror(errno));
      return false;
    }
...
static bool replay_realloc(ck_ht_t *htp, replay_stats_t* statsp, const uchar* buffer, size_t buffersz) {
  ...
  if (success) {
    ...
    if (ptr_in == NULL) {
      ...
      val_old = table_get(htp, ptr_in);

      if (val_old == NULL) {
	fprintf(stderr, "replay_realloc: Failed to find %p in the htp\n", ptr_in);
    ...
    if (sz == 0) {
      ...
      table_remove(htp, ptr_in);

      /* could assert that key is not in the htp */
      if ( ! table_insert(htp, ptr_out, val_new) ) {
	fprintf(stderr, "replay_realloc: Could not insert %p => %p into the htp: %s\n", ptr_out, val_new, strerror(errno));
	return false;
      }
	...
	/* either way still need to update the htp */
...
static bool replay_free(ck_ht_t *htp, replay_stats_t* statsp, const uchar* buffer, size_t buffersz) {
  ...
  if (success) {
    ...
    if (key == NULL) {
      ...
      val = table_get(htp, key);
      if (val != NULL) {

	_r_free(statsp, val);
	
	table_remove(htp, key);

      } else {
	/* this is a pretty common occurence */
	if (!silent_running) {
	  fprintf(stderr, "replay_free: Failed to find %p in the htp\n", key);
	}
	return true;
      }
    }

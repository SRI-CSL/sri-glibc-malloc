...
//iam #include "postgres.h"

//iam #include "access/xact.h"
//iam #include "storage/shmem.h"
//iam #include "storage/spin.h"
//iam #include "utils/dynahash.h"
//iam #include "utils/memutils.h"
...
struct HASHHDR
{
  ...
  pthread_mutex_t		mutex;			/* unused if not partitioned table */
...
struct HTAB
{
  ...
  memcxt_p  hcxt;			/* memory context if default allocator used */
 ...
 */
static memcxt_p CurrentDynaHashCxt = NULL;
...
hash_create(const char *tabname, long nelem, HASHCTL *info, int flags)
{
  ...
  if (!init_htab(hashp, nelem))
	fprintf(stderr, "failed to initialize hash table \"%s\"", hashp->tabname);
...
init_htab(HTAB *hashp, long nelem)
{
  ...
  if (IS_PARTITIONED(hctl))
	pthread_mutex_init(&hctl->mutex, NULL);
  ...
  if (hashp != NULL)
	{
	   ...
	   */
	  //iam MemoryContextDelete(hashp->hcxt);
	}
}
							...
							bool *foundPtr)
{
  ...
  switch (action)
	  ...
	  if (currBucket != NULL)
		{
		  ...
		  if (IS_PARTITIONED(hctlv))
			pthread_mutex_lock((pthread_mutex_t*)&hctlv->mutex);
		  ...
		  *prevBucketPtr = currBucket->link;
		  ...
		  if (IS_PARTITIONED(hctlv))
			pthread_mutex_unlock((pthread_mutex_t*)&hctlv->mutex);
	...
	case HASH_ENTER:
	  ...
	  if (hashp->frozen)
		fprintf(stderr, "cannot insert into frozen hashtable \"%s\"",
			 hashp->tabname);
  ...
  fprintf(stderr, "unrecognized hash action code: %d", (int) action);
...
hash_seq_init(HASH_SEQ_STATUS *status, HTAB *hashp)
{
  ...
  //iam   if (!hashp->frozen) register_seq_scan(hashp);
}
...
{
  //iam if (!status->hashp->frozen) deregister_seq_scan(status->hashp);
}
...
{
  if (hashp->isshared)
	fprintf(stderr, "cannot freeze shared hashtable \"%s\"", hashp->tabname);
  //iam  if (!hashp->frozen && has_seq_scans(hashp)) fprintf(stderr, "cannot freeze hashtable \"%s\" because it has active scans", hashp->tabname);
  hashp->frozen = true;
}
  ...
  if (p != NULL)
	{
	  ...
	  //iam assert(hashp->alloc == DynaHashAlloc);
	  //iam pfree(old_p);
...
hash_corrupted(HTAB *hashp)
{
   ...
   */
  if (hashp->isshared)
	fprintf(stderr, "hash table \"%s\" corrupted", hashp->tabname);
  else
	fprintf(stderr, "hash table \"%s\" corrupted", hashp->tabname);
}

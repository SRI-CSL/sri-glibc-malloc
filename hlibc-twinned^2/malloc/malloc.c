...
static inline void set_inuse_bit_at_offset(mstate av, chunkinfoptr _md_p,  mchunkptr p, size_t s)
{
  ...
  assert(_md_p->md_next == _md_next_chunk);
...
_int_free (mstate av, chunkinfoptr _md_p, mchunkptr p, bool have_lock, bool freshheap)
{
  ...
  */
    ...
    */
      ...
      _md_temp = av->_md_top;
      INTERNAL_SIZE_T topsize = chunksize (_md_temp);
      topend = (char *) (chunk_at_offset (topchunk, topsize));

      check_top(av); // iam: the next checktop fails occasionally.
      //does this one?

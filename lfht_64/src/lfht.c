...
bool lfht_insert(lfht_t *ht, uintptr_t key, uintptr_t val){
  ...
  if(ht != NULL  && key != 0){
    ...
    while (true) {
      ...
      if(entry.key == 0){
	if(cas_64((volatile uint64_t *)&(ht->table[i].key), *((uint64_t *)&(entry.key)), *((uint64_t *)&(desired.key)))){
	  //iam: discuss
	  ht->table[i].val = val;
	  return true;
	} else {
...
bool lfht_update(lfht_t *ht, uintptr_t key, uintptr_t val){
  ...
  if(ht != NULL  && key != 0){
    ...
    while (true) {
      ...
      if(entry.key == key){
	if(cas_64((volatile uint64_t *)&(ht->table[i].val), *((uint64_t *)&(entry.val)), *((uint64_t *)&(desired.val)))){
	  return true;
	} else {
	  continue;
	}
